function arrival_times = s2s_rollback(num_of_mols, reciever_radius, ...
...
for i = 1:time/step
    ...
    %Rollback operation. Molecules that hit the transmitter gets rolled
    %back.
    mol_matrix(hits_t, :) = mol_matrix(hits_t, :) - (movements(hits_t, :) + (flow * step));
